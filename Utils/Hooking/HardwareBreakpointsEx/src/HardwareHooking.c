...
VOID SetFunctionArgument(IN PCONTEXT pThreadCtx, IN ULONG_PTR uValue, IN DWORD dwParmIndex) {
	...
	// else more arguments are pushed to the stack
	return *(ULONG_PTR*)(pThreadCtx->Rsp + (dwParmIndex * sizeof(PVOID))) = uValue;
#else
	return *(DWORD_PTR*)(pThreadCtx->Esp + (dwParmIndex * sizeof(PVOID))) = uValue;
#endif // _WIN64
}
...
LONG ExceptionHandlerCallbackRoutine(IN PEXCEPTION_POINTERS ExceptionInfo)
{
    ...
    while (TempObject != NULL){
#ifdef _WIN64
	pReturn = ExceptionInfo->ContextRecord->Rip;
#elif _WIN32
	pReturn = ExceptionInfo->ContextRecord->Eip;
#endif // _WIN64

		// Search for the detour function pointer of the breakpoint triggered if the node is not processed yet
		if (TempObject->Address == pReturn && !TempObject->Processed) {
